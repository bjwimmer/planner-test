const BUILD_VERSION = 'v33-test-guarded-wide';
...
function initLifeMap(){
  ...
  function goalRow(hKey, domain, g){
    ...
    // - Week column: only RIGHT arrow
    // - Month column: both arrows
    // - 3 Months column: only LEFT arrow
    const gid = escapeAttr(String(g.id));
    const leftBtn = (hKey!=="week")
      ? `<button type="button" class="btn arrow" title="Move sooner (toward This Week)" data-left="${gid}" data-h="${hKey}" data-d="${escapeAttr(domain)}">&#8592;</button>`
      : ``;
    const rightBtn = (hKey!=="quarter")
      ? `<button type="button" class="btn arrow" title="Move later (toward 3 Months)" data-right="${gid}" data-h="${hKey}" data-d="${escapeAttr(domain)}">&#8594;</button>`
      : ``;
    return `
  ...
  function render(){
    ...
    // Promote / demote
  // Move arrows — delegated so they keep working after any re-render.
  root.addEventListener("click", (e)=>{
    const btn = e.target.closest("button[data-left],button[data-right]");
    if(!btn || !root.contains(btn)) return;
    if(btn.hasAttribute("data-left")){
      moveGoal(btn.getAttribute("data-left"), btn.getAttribute("data-h"), btn.getAttribute("data-d"), "sooner");
      return;
    }
    if(btn.hasAttribute("data-right")){
      moveGoal(btn.getAttribute("data-right"), btn.getAttribute("data-h"), btn.getAttribute("data-d"), "later");
    }
  });
